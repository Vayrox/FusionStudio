...
STEP4_FUSION_DESIGN_BLEND = (
    ...
    "PALETTE BOLDNESS (target the 'shiny variant' aesthetic - high contrast wins): "
    "the blended palette should be DRAMATIC, not muted. If the distinctive traits "
    "hint at shadow / void / ethereal / legendary / mythic / spectral, dramatic "
    "palette transformations are ENCOURAGED: jet-black + accent-glow lines, "
    "molten-gold filigree edges, chrome-iridescent shimmer, deep obsidian with "
    "ember-veins, lunar-silver with crystal pulse-points. The palette must still "
    "trace back to the originals (e.g. Mewtwo's purple becomes shiny obsidian-"
    "violet with gold edge-glow, Rayquaza's green becomes black-emerald with gold "
    "ring-markings), but treat the originals' colors as INSPIRATION not strict "
    "limits. Edge-light rim should make the silhouette pop against the background.\n\n"
    ...
    "doll. Imagine a hero-shot from a movie poster or a premium collectible's "
    "showcase angle.\n\n"
    "RENDER QUALITY (PREMIUM COLLECTIBLE STATUE - critical): the final image must "
    "read as a HIGH-END Pokemon Center premium collectible statue or a Bandai "
    "Tamashii / Sideshow Collectibles figure shot in a product photography studio - "
    "NOT a generic 3D render. Required quality markers:\n"
    "  - Surface micro-detail visible: individual fur strands at the silhouette "
    "edge, scale-by-scale rendering with subsurface translucency on backlit areas, "
    "fabric weave pattern if any cloth/wings/membranes, brushed-metal anisotropy "
    "if armor, crystal facets show refraction with prismatic edge-glow, skin pores "
    "and wet eye-glints rendered.\n"
    "  - Lighting: studio-portrait three-point setup - soft key light from camera-"
    "front, cool blue or warm gold rim-light from behind/side to separate from "
    "background, subtle bounce fill. Edge-rim makes the silhouette POP. NOT flat "
    "front-light.\n"
    ...
    "  - Materials: PBR-grade. Skin has subsurface scattering, scales are "
    "translucent at edges, metal has crisp anisotropic highlights, fur has hair-"
    "card-rendering not blob-texture, eyes have wet-glint + iris depth + tiny "
    "catchlight.\n"
    "  - Sharpness + DOF: razor-sharp on the creature, gentle photographic depth-"
    "of-field that slightly softens the background but keeps the platform readable.\n"
    "  - NO stylized illustration, NO 2D digital painting, NO concept art look, "
    "NO cel-shading, NO toon-shader edges. The creature must look like a real "
    "physical sculpture you could hold."
)
...
STEP4_FUSION_DESIGN_UNIQUE = (
    ...
    "PALETTE BOLDNESS (target the 'shiny variant' aesthetic - high contrast wins): "
    "the new palette should be DRAMATIC and high-impact. Strong directions are "
    "ENCOURAGED: jet-black + accent-glow lines, molten-gold filigree edges, chrome-"
    "iridescent shimmer, deep obsidian with ember-veins, lunar-silver with crystal "
    "pulse-points, void-purple with electric-cyan plasma trim. Edge-light rim "
    "should make the silhouette pop against the background.\n\n"
    "POWER POSE (mandatory): the fusion stands in a CONFIDENT CINEMATIC stance, "
    "NOT a flat neutral stand. Pick one: power-stance with weight on one leg + "
    "shoulders squared + chest forward; mid-action gesture with one hand/claw "
    "raised or charging energy; dramatic three-quarter angle with the head turned "
